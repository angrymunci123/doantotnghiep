    ...
    <div class="h-100 rounded p-4" style="background-color: white">
        ...
        <table>
            <tr>
                ...
                <td><br></td>
                <td>
                    <form method="get" action="/storeIndex/order_history/confirmed">
                        <button name="controller=create" class="btn btn-primary">Confirmed</button>
                    </form>
                </td>
                <td><br></td>
                <td>
                    <form method="get" action="/storeIndex/order_history/shipping">
        ...
        <table class="table table-bordered">
            ...
            @foreach($order_history as $pnd)
                <tr>
                    ...
                    <td>{{$pnd->customer_name}}</td>
                    <td>{{$pnd->status}}</td>
                    <td style="width:100px">
                        @if($pnd->status == "Pending")
                            <form action="/storeIndex/order_history/view_pending_order/order_id={{$pnd->order_id}}" method="GET">
                        ...
                        @if($pnd->status != "Pending" && $pnd->status != "Canceled")
                            <form action="/storeIndex/order_history/view_order/order_id={{$pnd->order_id}}" method="GET">
                                <button type="submit" class="btn btn-warning rounded-1" style="width:100px">View</button>
                            </form>
                        @endif

                        @if($pnd->status == "Canceled")
                            <form action="/storeIndex/order_history/view_canceled_order/order_id={{$pnd->order_id}}" method="GET">
                                <button type="submit" class="btn btn-warning rounded-1" style="width:100px">View</button>
                            </form>
                        @endif
                    </td>
